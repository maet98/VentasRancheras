...
import { Grid, makeStyles, Card,CardActionArea,CardContent,Typography,CardActions, Select, MenuItem } from '@material-ui/core';
...
export default function VerClientes(props) {
	...
	useEffect(() =>{
		api.employee().getClients(id).then(ans => {
			var arr = {};
		...
		api.client().getAvailable().then(ans => {
	...
	const onSubmit = () =>{
		...
		api.route().create(data).then((ans) =>{
			console.log(ans);
		}).catch(err =>{
			console.log(err);
		})
					...
					<h3>Clientes Displonibles:</h3>
					{
						...
						available.map((ac, id)=>{
							return (
								...
								<CardActions>
									<Button size="small" color="primary" onClick={() =>handleChange(ac.Id)}>
									Asignar?
									</Button>
								</CardActions>
								</Card>
					...
					<h3>Clientes de este vendedor:</h3>
					{
						...
						clients.map((ac, id)=>{
							return (
								...
								<CardActions>
									<Button disabled={original[ac.Id]} size="small" color="primary" onClick={() =>otherWay(ac.Id)}>
									Devolver?
									</Button>
									<Select
									id={ac.Id}
